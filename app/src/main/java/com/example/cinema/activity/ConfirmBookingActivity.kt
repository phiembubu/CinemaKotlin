...
import com.example.cinema.adapter.*
...
class ConfirmBookingActivity : AppCompatActivity() {
    ...
    private fun getListRoomLocal() : List<Room> {
        val list: MutableList<Room> = ArrayList()
    ...
    private fun getListTimeLocal(roomId: Int): List<SlotTime> {
        val list: MutableList<SlotTime> = ArrayList()
    ...
    private fun getListSeatLocal(time: SlotTime): List<SeatLocal> {
        ...
        val list: MutableList<SeatLocal> = ArrayList()
        if (timeFirebase?.seats != null) {
            for (seat in timeFirebase.seats!!) {
                try{
                    val vip = seat.isVip
                    val seatLocal = SeatLocal(seat.id, seat.title,
                        seat.isSelected, time.roomId, time.id, isVip = vip)
                    list.add(seatLocal)
                }catch (e: Exception){
                    Log.e("TAG", "getListSeatLocal: " + e.message)
                    val seatLocal = SeatLocal(seat.id, seat.title,
    ...
    private fun getListFoodAndDrink() {
        MyApplication[this].getFoodDatabaseReference().addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                if (mListFood != null) {
                    mListFood!!.clear()
                } else {
    ...
    private fun getListSeatChecked() : List<SeatLocal> {
        val listSeatChecked: MutableList<SeatLocal> = ArrayList()
        if (mListSeats != null) {
    ...
    private fun getListVipSeatChecked() : List<SeatLocal> {
        val listSeatChecked: MutableList<SeatLocal> = ArrayList()
        if (mListSeats != null) {
            for (seat in mListSeats!!) {
                try{
                    val vip = seat.isVip
                    if (seat.isChecked && vip) {
                        listSeatChecked.add(seat)
                    }
                }catch (e: Exception){
                    Log.e("TAG", "getListVipSeatChecked: " + e.message)
                }
    ...
    private fun getListFoodSelected() : List<Food> {
        val listFoodSelected: MutableList<Food> = ArrayList()
        if (mListFood != null) {
    ...
    private fun getTotalAmount() : Int {
        ...
        var countBooking = 0
        var countVipBooking = 0
        try {
            countBooking = getListSeatChecked().size
            countVipBooking = getListVipSeatChecked().size
        } catch (e: Exception) {
            e.printStackTrace()
        }
        val priceMovie = countBooking * mMovie?.price!! + countVipBooking * 5000
